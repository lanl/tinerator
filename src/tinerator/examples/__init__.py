...
# Returns the absolute path to the examples/data/ directory.
DATA_DIR = _get_data("")
...
class NewMexico(object):
    ...
    def __init__(self):
        self.dem = load_raster(
            _get_data("new_mexico/rasters/USGS_NED_13_n37w108_Clipped.tif")
        )
        self.flowline = load_shapefile(
            _get_data("new_mexico/shapefiles/NHDFlowline/NHDFlowline.shp")
        )
        self.boundary = load_shapefile(
            _get_data("new_mexico/shapefiles/WBDHU12/WBDHU12.shp")
        )
