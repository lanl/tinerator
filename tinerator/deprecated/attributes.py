...
def _map_function_to_attribute(
    lg,
    stacked_mesh: str,
...
def _add_attribute(
    ...
    if attribute_name is not None:
        cfg.log.info(
            'Adding attribute "'
            + attribute_name
            + '" to mesh (type: %s)' % dtype
        )
    else:
        cfg.log.info(
            'Adding attribute "MATERIAL_ID" to mesh (type: %s)' % dtype
        )
    ...
    mo_extrude = mo_quad.extrude(10000.0)
    ...
    if layers is None:
        ...
        stacked_mesh.interpolate(
            "map", attribute_name, mo_extrude, "id_strat", stride=[1, 0, 0]
        )
    else:
        # Conform scalar to type(list<int>)
        if isinstance(layers, (int, float)):
        ...
        for layer in layers:
            ...
            # Set att to 2 *only* for the current layer we're on
            stacked_mesh.setatt(
                "elttmp", 2, stride=[v * (layer - 1), v * layer]
            )
