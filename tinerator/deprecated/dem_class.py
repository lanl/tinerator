...
class DEM:
    ...
    def __init__(self, filepath: str, lagrit_exe: str = None):
        ...
        self.yll_corner = (
            self.dem.geotransform[3] - self.nrows * self.cell_size
        )
        self.mask = (
            self.dem == self.dem.no_data
    ...
    def __del__(self):
        if not cfg.KEEP_LAGRIT_LOGS:
            util.cleanup(
                ["logx3dgen", "outx3dgen", "lagrit.log", "lagrit.out"]
            )
    ...
    def watershed_delineation(
        ...
        * Freeman
        ...
        if (exponent is not None) and (method not in ["Holmgren", "Freeman"]):
            cfg.log.warn(
                "Ignoring exponent: exponent is only valid "
                + "for Holmgren/Freeman methods"
            )

        self.accumulation_matrix = delin.watershedDelineation(
            self.dem, method=method, exponent=exponent
        )
        ...
        self.feature = delin.getFeatureTrace(
            self.accumulation_matrix, feature_threshold=threshold
        )

        if np.size(self.feature) == 0:
            raise ValueError(
                "Feature trace is empty. "
                + "Try setting a lower threshold or "
                + "using a different method."
            )

        self.feature = util.xyVectorToProjection(
            self.feature,
            self.cell_size,
            self.xll_corner,
            self.yll_corner,
    ...
    def set_river_network_from_raster(
        self, raster: np.ndarray, threshold: float
    ) -> None:
        """
        ...
        self.feature = delin.getFeatureTrace(
            raster, feature_threshold=threshold
        )
    ...
    def import_river_network(self, shp_filepath: str) -> None:
        ...
        raster = util.rasterize_shapefile_like(
            shp_filepath, self.metadata["filename"]
        )
    ...
    def __watershed_delin_as_interactive(self, threshold, method):
        ...
        def __fnc_driver(threshold, method):
            self.watershed_delineation(
                threshold, method=method, interactive=False
            )
        ...
        step = (max_accum - min_accum) / 100.0
        ...
        interact_manual(
            ...
            threshold=widgets.FloatSlider(
                min=min_accum, max=max_accum, step=step, value=threshold
            ),
    ...
    def _generate_boundary(self, distance: float, rectangular: bool = False):
        ...
        self.boundary = boundary.squareTraceBoundary(
            self.dem, self.no_data_value, dist=distance
        )

        self.boundary = util.xyVectorToProjection(
            self.boundary,
            self.cell_size,
            self.xll_corner,
            self.yll_corner,
    ...
    def build_uniform_triplane(
        ...
        if interactive:
            if cfg.IN_NOTEBOOK:
                return self.__interactive_triplane_uniform_driver(
                    edge_length,
                    smooth_boundary,
                    flip,
                    apply_elevation,
        ...
        self._generate_boundary(
            boundary_distance, rectangular=rectangular_boundary
        )
        self._surface_mesh = mesh._uniform_surface_mesh(
            self.lg,
            self.boundary,
            outfile=outfile,
        ...
        if apply_elevation:
            mesh._intrp_elevation_to_surface(
                self.lg,
                self,
                self._surface_mesh,
                outfile=outfile,
    ...
    def build_refined_triplane(
        ...
        if interactive:
            if cfg.IN_NOTEBOOK:
                return self.__interactive_triplane_refined_driver(
                    min_edge_length,
                    max_edge_length,
                    smooth_boundary,
                    flip,
        ...
        self._surface_mesh = mesh._refined_surface_mesh(
            self.lg,
            self.boundary,
            feature,
    ...
    def build_layered_mesh(self, layers, matids=None, outfile: str = None):
        """
        ...
        if self._surface_mesh is None:
            raise ValueError(
                "A surface mesh is required - run triplane generation"
            )

        self._stacked_mesh = mesh._stacked_mesh(
            self.lg, self._surface_mesh, outfile, layers, matids=matids
        )
    ...
    def add_attribute(
        self, data, layers=None, attribute_name=None, outfile=None, dtype=None
    ):
        """
        ...
        self._stacked_mesh = attrib._add_attribute(
            self.lg,
            data,
            self._stacked_mesh,
    ...
    def map_function_to_attribute(
        ...
        attrib._map_function_to_attribute(
            self.lg,
            self._stacked_mesh,
            self.number_of_layers,
            layers=layers,
    ...
    # =====================================
    def __interactive_triplane_uniform_driver(
        self,
        edge_length,
        smooth_boundary,
        flip,
        ...
        min_edge = 0.0
        step = (max_edge - min_edge) / 100.0

        if edge_length is None:
            edge_length = max_edge / 2.0

        edge_length = max(min_edge, min(max_edge, edge_length))

        interact_manual(
            __fnc_driver,
            edge_length=widgets.FloatSlider(
                min=min_edge, max=max_edge, step=step, value=edge_length
            ),
    ...
    def __interactive_triplane_refined_driver(
        self,
        min_edge,
        max_edge,
        smooth_boundary,
        ...
        interact_manual(
            ...
            slope=widgets.FloatSlider(min=0.0, max=2.5, step=0.1, value=0.5),
            smooth_boundary=widgets.Checkbox(
                value=False, description="Smooth DEM boundary", disabled=False
            ),
            flip=["y", "x", "xy"],
            apply_elevation=fixed(apply_elevation),
            outfile=fixed(outfile),
