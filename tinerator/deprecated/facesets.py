...
def write_facesets(dem_object, facesets):
    ...
    try:
        ...
        mo_surf = dem_object.lg.extract_surfmesh(
    ...
    if bool(sidesets):
        new_fs = __driver_sidesets(
            dem_object.lg, mo_surf, has_top, boundary_file, full_sidesets
        )
...
def __selectFacesetsFromBoundary(dem):
    ...
    extent = (
        dem.xll_corner,
        dem.ncols * dem.cell_size + dem.xll_corner,
        dem.yll_corner,
        dem.nrows * dem.cell_size + dem.yll_corner,
    )
    ...
    def reset_pts(event):
        ...
        ax_status.clear()
        ax_status.imshow(
            dem_raster, zorder=1, extent=extent, cmap=plt.cm.cubehelix
        )
        ax.clear()
        mask[:] = True
        ax.plot(
            dem.boundary[:, 0][mask], dem.boundary[:, 1][mask], "o", picker=5
        )
    ...
    def save_pts(event):
        ...
        if _start > _end:
            ...
            ax_status.scatter(
                dem.boundary[_seq[0][0] : _seq[0][1], 0],
                dem.boundary[_seq[0][0] : _seq[0][1], 1],
                c=np.repeat(colors[color_idx], (_seq[0][1] - _seq[0][0])),
            )
            ax_status.scatter(
                dem.boundary[_seq[1][0] : _seq[1][1], 0],
                dem.boundary[_seq[1][0] : _seq[1][1], 1],
                c=np.repeat(colors[color_idx], (_seq[1][1] - _seq[1][0])),
            )
            ...
            ax_status.scatter(
                dem.boundary[_start:_end, 0],
                dem.boundary[_start:_end, 1],
                c=np.repeat(colors[color_idx], _end - _start),
            )
