...
def _intrp_elevation_to_surface(
    lg,
    dem_object,
    triplane,
    flip: str = "y",
    ...
    lower_left_corner = [dem_object.xll_corner, dem_object.yll_corner, 0.0]
    ...
    if smooth_boundary:
        _dem = util._smooth_raster_boundary(
            _dem, width=4, no_data_value=np.nan
        )

    ind = nd.distance_transform_edt(
        np.isnan(_dem), return_distances=False, return_indices=True
    )
    _dem = _dem[tuple(ind)]
    ...
    comp = "le" if dem_object.no_data_value <= 0.0 else "ge"
    dud_value = dem_object.no_data_value - np.sign(dem_object.no_data_value)
    pduds = tmp_sheet.pset_attribute(
        "zic", dud_value, comparison=comp, name="pduds"
    )
    ...
    try:
        ...
        _err = (
            "Caught an unknown exception. Most likely, this is related to"
...
def _stacked_mesh(lg, surface_mesh, outfile: str, layers: list, matids=None):
    """
    ...
    stack_files = [
        "layer%d.inp" % (len(layers) - i) for i in range(len(layers))
    ]
    ...
    stack = lg.create()
    stack.stack_layers(
        stack_files, flip_opt=True, nlayers=nlayers, matids=matids
    )
...
def __generateSingleColumnPrism(
    lg,
    infile: str,
    outfile: str,
    layers: list,
