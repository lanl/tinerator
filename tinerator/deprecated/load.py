...
def __download_dem(
    ...
    if shapefile is None:
        if bounds is None:
            raise RuntimeError(
                "Parameters bounds or shapefile must be defined!"
            )
        ...
        if margin is not None:
            if isinstance(margin, str):
                margin_percent = float(margin[:-1]) / 100.0
...
def from_matrix(
    ...
    **Note: no_data_value cannot be np.nan.**
    ...
    """
    shp = np.shape(array)
    hdr = "ncols {0}\nnrows {1}\nxllcorner {2}\nyllcorner {3}\ncellsize {4}\nnodata_value {5}".format(
        shp[1], shp[0], xllcorner, yllcorner, cell_size, no_data_value
    )
...
def from_coordinates(bounding_box, outfile=None, SRTM_30=True):
    ...
    `tinerator.DEM` instance
    """
    return __download_dem(
        bounds=bounding_box, outfile=outfile, SRTM_30=SRTM_30
    )
...
def from_shapefile(shapefile, outfile=None, SRTM_30=True):
    ...
    """
    return __download_dem(
        shapefile=shapefile, outfile=outfile, SRTM_30=SRTM_30, crop=True
    )
