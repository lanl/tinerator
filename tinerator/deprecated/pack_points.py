...
def cullTrianglesToAlphaHull(
    ...
    for i in range(0, len(triangles)):
        ...
        centroidx = (
            points[tri[0]][0] + points[tri[1]][0] + points[tri[2]][0]
        ) / 3.0
        centroidy = (
        ...
        if isPointOutsidePolygon(
            boundary, centroid, [reference_point_x, centroidy]
        ):
...
def triangulateNodes(
    ...
    points -= points.mean(
        axis=0
    )  # Center points around origin to avoid rounding errors
    ...
    if preserve_boundary == True:
        triangles = cullTrianglesToAlphaHull(
            nodes, triangles, boundary_vertices, ncols, cell_size, xll_corner
        )
...
def CellsInCircle(maxDiameter, cellSize):
    ...
    arrayCellsInCircle = np.zeros(
        (int(maxCells) + 1, 3, int(maxCells * maxCells) + 1), dtype=int
    )
    ...
    for d in range(int(minCells) - 1, int(maxCells) + 1):
        ...
        for y in range(int(lowerCell), int(upperCell)):
            for x in range(int(lowerCell), int(upperCell)):
                ...
                if skipX2 == False:
                    cellCenterDistance = (
                        ((float(x) * cellSize) ** 2.0)
                        + ((float(y) * cellSize) ** 2.0)
                    ) ** 0.5
                    ...
                    if cellCenterDistance <= radius:
                        ...
                        cellCenterDistance = (
                            (((np.abs(float(x)) + 1) * cellSize) ** 2.0)
                            + ((float(y) * cellSize) ** 2.0)
                        ) ** 0.5
                        if cellCenterDistance > radius:
                            arrayCellsInCircle[d][0][cellCount] = 1
                        cellCenterDistance = (
                            ((float(x) * cellSize) ** 2.0)
                            + (((np.abs(float(y)) + 1.0) * cellSize) ** 2.0)
                        ) ** 0.5
                        if cellCenterDistance > radius:
                            arrayCellsInCircle[d][0][cellCount] = 1
...
def PlacePoints(
    ...
    if False:  # boundary is not None:
        ...
        for i in range(0, len(points)):
            ...
            row = int(
                round(convertProjectiontoX(points[i][0], cellSize, xllCorner))
            )
            col = int(
                round(
                    convertProjectiontoY(
                        points[i][1], cellSize, yllCorner, nRows
                    )
                )
            )
    ...
    for y in range(0, nRows):
        for x in range(0, nCols):
            ...
            distance = float(arrayDistance[x][y]) * 10.0 - 1
            ...
            if int(arrayDistance[x][y]) == int(noDataValue):
                ...
                diameterAsCell = (
                    int((distance - minDistance) / (distanceRange) * edgeRange)
                    + minEdge
                )
    ...
    # Second
    for (iteration, xy) in enumerate(
        random.sample(range(0, nCols * nRows), nCols * nRows)
    ):
        ...
        if (
            float(diameterAsCell) * float(cellSize)
        ) != noDataValue and diameterAsCell != -999:
            diameterAsCell = int(
                round(float(arrayCellDistance[x][y]) / float(cellSize))
            )
    ...
    points[:, 1] = yVectorToProjection(
        points[:, 1], cellSize, yllCorner, nCols
    )
    return points
