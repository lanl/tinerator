...
try:
    ...
    topocmap = pycpt.load.cmap_from_cptcity_url(
        "wkp/schwarzwald/wiki-schwarzwald-cont.cpt"
    )
except:
    topocmap = "Spectral_r"
...
def preview_accumulation(dem_object, method: str = "D8"):
    ...
    im = ax.imshow(
        accum, norm=LogNorm(vmin=0.01, vmax=np.max(accum)), extent=extent
    )
...
def plot_feature(dem_object):
    ...
    if dem_object.feature is None:
        raise ValueError(
            "DEM has no feature - watershed delination must be run"
        )
...
def preview_boundary(
    dem_object, distance: float, rectangular: bool = False, interactive=False
):
    """
    ...
    if interactive and cfg.IN_NOTEBOOK:
        ...
        interact_manual(
            ...
            distance=widgets.FloatSlider(
                min=1, max=delta_x, step=5, value=distance
            ),
...
def plot_triplane(dem_object):
    ...
    print(
        "WARNING: This function may not render correctly.\n"
