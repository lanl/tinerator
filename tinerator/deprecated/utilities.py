...
def convertXtoProjection(x, cellSize, xllCorner):
    return xllCorner + (float(x) * cellSize) - (cellSize / 2.0)
...
def convertProjectiontoX(x, cellSize, xllCorner):
    return (cellSize + 2.0 * float(x) - 2.0 * xllCorner) / (2.0 * cellSize)


def convertProjectiontoY(y, cellSize, yllCorner, nRows):
    return (yllCorner - y) / cellSize + nRows + 1.0 / 2.0
...
def _smooth_raster_boundary(
    raster: np.ndarray, width: int, no_data_value: float = np.nan
):
    """
    ...
    edges = skimage.transform.resize(
        mask,
    ...
    edges = np.pad(
        edges, pad_width=int(width / 2), mode="constant", constant_values=True
    )
    edges = ~edges & mask
