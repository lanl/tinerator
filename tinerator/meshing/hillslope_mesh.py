...
def create_hillslope_mesh(z_data: np.ndarray, x_coords: np.ndarray = None, y_coords: np.ndarray = None, crs: CRS = None):
    """
    ...
    for j in range(z_rows - 1):
        for i in range(z_cols - 1):
            quad = [i+j*z_cols, i+j*z_cols+1, i+1+(j+1)*z_cols, i+(j+1)*z_cols]
            quads.append(quad)
