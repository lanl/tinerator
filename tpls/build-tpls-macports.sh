...
# Macport compiler names
# - could automate, version is not critical.
GCC_NAME=gcc-mp-10
GCXX_NAME=g++-mp-10
GFORTRAN_NAME=gfortran-mp-10
...
build_jigsaw() {
    ...
    CC="${CC:-$(which $GCC_NAME)}"
    CXX="${CXX:-$(which $GCXX_NAME)}"
    FC="${FC:-$(which $GFORTRAN_NAME)}"
...
#   Assumes netcdf and hdf5 are already installed in python environment
#
build_exodus() {
    _cwd=$(pwd)
